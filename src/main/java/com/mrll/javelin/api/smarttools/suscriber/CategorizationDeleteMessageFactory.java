...
public class CategorizationDeleteMessageFactory implements MessageFactory<MetadataUpdateEvent> {
    private static final Logger LOG = LoggerFactory.getLogger(CategorizationDeleteMessageFactory.class);
    ...
    public MetadataUpdateEvent createMessage(byte[] input) {
        try {
            ...
            String message = "message=Delete jsonProcessingErrorHandlingMessage";
            LOG.warn(message, ex);
            throw new FatalSmartSortException(message, ex);
        } catch (IOException ex) {
            String message = "message=Delete ioErrorHandlingMessage";
            LOG.warn(message, ex);
            throw new FatalSmartSortException(message, ex);
        }
    }
